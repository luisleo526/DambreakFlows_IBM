...
do iter = 1, 20
    ...
    do id = 0, p%glb%threads-1
        ...
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
            ...
            if( p%glb%method == 4) plam = plam * p%of(id)%loc%normals%curv%now(i,j,k)
    ...
    !$omp parallel do private(i,j,k,plam)
    do id = 0, p%glb%threads-1

        !$omp parallel do num_threads(p%glb%nthreads) collapse(3) private(i,j,k,plam)
        do k = p%of(id)%loc%ks, p%of(id)%loc%ke
        do j = p%of(id)%loc%js, p%of(id)%loc%je
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
            ...
            p%of(id)%loc%phi%now(i,j,k) = p%of(id)%loc%phi%now(i,j,k) + plam * p%of(id)%loc%delta%now(i,j,k) * p%of(id)%loc%grad%now(i,j,k)
